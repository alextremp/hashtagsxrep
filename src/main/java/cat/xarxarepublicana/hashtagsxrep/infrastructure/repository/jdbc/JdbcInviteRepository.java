...
public class JdbcInviteRepository implements InviteRepository {
  ...
  public InviteGroup loadInvitesForPoll(String pollId) {
    ...
    return new InviteGroup(
        inviteList.stream()
            .filter(invite -> Invite.REASON_ADMIN.equals(invite.getReason()))
            .collect(Collectors.toList()),
        inviteList.stream()
            .filter(invite -> Invite.REASON_TAGGER.equals(invite.getReason()))
            .collect(Collectors.toList()),
        inviteList.stream()
            .filter(invite -> Invite.REASON_INFLUENCE.equals(invite.getReason()))
            .collect(Collectors.toList()),
        inviteList.stream()
            .filter(invite -> Invite.REASON_SCORE.equals(invite.getReason()))
            .collect(Collectors.toList()),
        inviteList.stream()
            .filter(invite -> Invite.REASON_RANDOM.equals(invite.getReason()))
            .collect(Collectors.toList())
    );
  }
}
